    ...
    <div class="col-12 col-md-4">
        <div class="card">
            <div class="card-header">
                <button type="button" class="btn btn-sm btn-light" onclick="window.location.href='{{ route('tgsperawat.index') }}'" style="margin-right: 7px"><i class="fa-fw fas fa-angle-left nav-icon"></i> Kembali</button>
                <h4>Form Tambah</h4>
            </div>
            <div class="card-body">
                <form class="form-auth-small" action="{{ route('logtgsperawat.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="logtgsperawat" class="table table-striped display">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>PERNYATAAN</th>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-footer">
            @if(count($list) > 0)
                <form action="{{ route('logtgsperawat.destroy', $item->id) }}" method="POST">
                    @method('DELETE')
                    @csrf
...
<div class="modal fade bd-example-modal-lg" id="editLog{{ $item->id }}" role="dialog" aria-labelledby="confirmFormLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        @if(count($list) > 0)
        {{ Form::model($item, array('route' => array('logtgsperawat.update', $item->id), 'method' => 'PUT')) }}
        <div class="modal-body">
            @csrf
            <label>Unit :</label>
            <select class="form-control selectric" name="unit" id="unit" required>
        ...
        <div class="modal-footer">
            <button class="btn btn-primary text-white btn-block" id="submit"><i class="fa-fw fas fa-save nav-icon text-white"></i> Submit</button>
        </div>
        {{ Form::close() }}
        @endif
      </div>
