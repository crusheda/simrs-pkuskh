                            ...
                            <td>
                                <center>
                                    <div class="btn-group" role="group">
                                        <button type="button" class="btn btn-success btn-sm" onclick="window.location.href='{{ url('regulasi/panduan/'. $item->id) }}'" data-toggle="tooltip" data-placement="bottom" title="DOWNLOAD"><i class="fa-fw fas fa-download nav-icon text-white"></i></button>
                                        @role('sekretaris-direktur')
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <form class="form-auth-small" action="{{ route('panduan.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-4">
...
<div class="modal fade bd-example-modal-lg" id="ubah{{ $item->id }}" role="dialog" aria-labelledby="confirmFormLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body">
            {{ Form::model($item, array('route' => array('panduan.update', $item->id), 'method' => 'PUT')) }}
                @csrf
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            -
                            @else
                                <b><u><a href="panduan/{{ $item->id }}">{{ substr($item->title,0,50) }}...</a></u></b><br><sub>Ukuran File : {{ number_format(Storage::size($item->filename) / 1048576,2) }} MB</sub>
                            @endif
                        </div>
                    </div>
                </div>
...
<div class="modal fade" id="hapus{{ $item->id }}" role="dialog" aria-labelledby="confirmFormLabel" aria-hidden="true">
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <p>Apakah anda yakin ingin menghapus Regulasi panduan <b>{{ $item->judul }}</b>?</p>
        </div>
        <div class="modal-footer">
