...
<div class="container">
    <div class="row">
        <div class="card" style="width: 100%">
            ...
            <div class="card-body">
                @can('imut_it')
                <form class="form-auth-small" action="{{ route('pilar.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                                ...
                                <tr>
                                    <td>
                                        @if ($item->jamselesai == null)
                                            <form action="{{ route('pilar.selesai', $item->id) }}" method="POST">
                                                @csrf
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#editPilar{{ $item->id }}">Ubah</button>
                                        <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#hapusPilar{{ $item->id }}">Hapus</button>
                                    </td>
                                </tr>
                                @endforeach
                                @else
...
<div class="modal fade bd-example-modal-lg" id="editPilar{{ $item->id }}" role="dialog" aria-labelledby="confirmFormLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        @if(count($list) > 0)
        {{ Form::model($item, array('route' => array('pilar.update', $item->id), 'method' => 'PUT')) }}
        <div class="modal-body">
            @csrf
