    ...
    <div class="row">
        <div class="card" style="width: 100%">
            <div class="card-header bg-dark text-white">
                ...
                <i class="fa-fw fas fa-wifi nav-icon text-info">

                </i> Indikator Mutu Jaringan
            ...
            <div class="card-body">
                @can('imut_it')
                <form class="form-auth-small" action="{{ route('it.jaringan.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                                ...
                                <tr>
                                    <td>
                                        @if ($item->jamselesai == null)
                                            <form action="{{ route('it.jaringan.selesai', $item->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="btn btn-success btn-sm">Selesai</button>
                                            </form>
                                        @endif
                                        {{-- <a class="btn btn-success btn-sm text-white" onclick="window.location.href='{{ route('jaringan.selesai', $item->id) }}'">
                                            Selesai
                                        </a> --}}
                                    </td>
                                    <td>{{ $item->namapi }}</td>
...
@foreach($list['show'] as $item)
<div class="modal fade bd-example-modal-lg" id="hapusjaringan{{ $item->id }}" role="dialog" aria-labelledby="confirmFormLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-footer">
            @if(count($list) > 0)
                <form action="{{ route('it.jaringan.destroy', $item->id) }}" method="POST">
                    {{-- <a class="btn btn-warning btn-sm" onclick="window.location.href='{{ route('imutjaringan.edit', $item->id) }}'">
...
@foreach($list['show'] as $item)
<div class="modal fade bd-example-modal-lg" id="editjaringan{{ $item->id }}" role="dialog" aria-labelledby="confirmFormLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">
        ...
        @if(count($list) > 0)
        {{ Form::model($item, array('route' => array('it.jaringan.update', $item->id), 'method' => 'PUT')) }}
        <div class="modal-body">
            @csrf
