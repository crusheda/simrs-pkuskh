...
<h4 class="fw-bold py-3 mb-4">
  <span class="text-muted fw-light">Profil /</span> Detail <strong>ID : {{ $list['show']->id }}</strong>
</h4>
...
<button class="btn btn-label-dark mb-3" onclick="window.location='{{ route('karyawan.index') }}'" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="right" data-bs-html="true" title="<i class='bx bx-chevron-left bx-xs' ></i> <span>Kembali ke Halaman Sebelumnya</span>"><i class="bx bx-chevron-left bx-sm ms-sm-n2"></i> Kembali</button>
<!-- Header -->
<div class="row">
  <div class="col-md-8">
    <div class="row">
            ...
            <div class="flex-grow-1 mt-3">
              <div class="d-flex align-items-md-end align-items-sm-start align-items-center justify-content-md-between justify-content-start mx-4 flex-md-row flex-column gap-4" style="margin-top: 1rem">
                <div class="user-profile-info">
                  <h4>Akun : {{ $list['show']->name }} <sup><i class="bx bx-badge-check text-primary"></i></sup></h4>
                  <ul class="list-inline mb-0 d-flex align-items-center flex-wrap justify-content-sm-start justify-content-center gap-2">
                    <li class="list-inline-item fw-semibold">
                      @foreach($list['role'] as $role) <span class="badge bg-secondary">{{ $role->name }}</span> @endforeach
                      <p></p>
                      <i class='bx bx-calendar'></i> Login terakhir : <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;@if (!empty($list['showlog'][1])) {{ \Carbon\Carbon::parse($list['showlog'][1]->log_date)->diffForHumans() }} @else - @endif
                      <p></p>
      ...
      <div class="card mb-4">
        <div class="card-body">
          ...
          <ul class="list-unstyled mb-4 mt-3">
            ...
            <li class="d-flex align-items-center mb-3"><i class="bx bx-star"></i><span class="fw-semibold mx-2">Jabatan:</span> <span>@foreach($list['role'] as $role) <span class="badge bg-secondary">{{ $role->name }}</span> @endforeach</span></li>
          </ul>
        </div>
      </div>
    </div>
  ...
  <div class="col-md-4">
    <div class="card mb-4">
      <div class="card-body">
        <h5 class="card-action-title mb-0"><i class='bx bx-list-ul me-2'></i>Alamat Sesuai KTP</h5><br>
