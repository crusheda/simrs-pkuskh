...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="card" style="width: 100%">
            ...
            <div class="card-body">
                    <form class="form-auth-small" action="{{ route('kepegawaian.fungsional.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <label>Keterangan :</label>
                        <input type="text" name="ket" class="form-control" placeholder="e.g. PIC" autofocus required>
                        <br>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-footer">
            @if(count($list) > 0)
                <form action="{{ route('kepegawaian.fungsional.destroy', $item->id) }}" method="POST">
                    @method('DELETE')
                    @csrf
...
<div class="modal fade bd-example-modal-lg" id="edit{{ $item->id }}" role="dialog" aria-labelledby="confirmFormLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        @if(count($list) > 0)
        {{ Form::model($item, array('route' => array('kepegawaian.fungsional.update', $item->id), 'method' => 'PUT')) }}
        <div class="modal-body">
            @csrf
            <label>Keterangan :</label>
            <input type="text" name="ket" value="{{ $item->ket }}" class="form-control" placeholder="e.g. PIC" autofocus required>
            <br>
            <label>Nominal :</label>
